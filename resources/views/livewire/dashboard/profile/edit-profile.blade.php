...
<div class="space-y-10 pb-20">
        ...
        x-init="
            ...
            Livewire.hook('request', ({ fail }) => {
                fail(({ status, content }) => {
                    ...
                    const errors = Object.keys(JSON.parse(content).errors ?? {});
                    if (errors.some(e => e.startsWith('form.name') || e.startsWith('form.email') || e.startsWith('form.username') || e.startsWith('form.website_url') || e.startsWith('form.bio'))) tab = 'identidade';
                    else if (errors.some(e => e.startsWith('form.links'))) tab = 'links';
                    else if (errors.some(e => e.startsWith('form.show_email') || e.startsWith('form.visibility') || e.startsWith('form.is_searchable') || e.startsWith('form.seo_'))) tab = 'privacidade';
                    else if (errors.some(e => e.startsWith('form.'))) tab = 'aparencia';
                });
            });
        "
    ...
    >
        ...
        <nav class="relative mb-8 flex gap-2 overflow-x-auto pb-1 lg:sticky lg:top-24 lg:mb-0 lg:flex-col lg:gap-1.5 lg:overflow-visible lg:pb-0">
            @php
                $navItems = [
                    ...
                    'aparencia' => ['label' => 'Aparência & Cores', 'icon' => 'palette', 'errors' => ['form.primary_color', 'form.accent_color', 'form.secondary_color', 'form.text_color', 'form.background_color', 'form.theme_mode', 'form.layout_type', 'form.button_style', 'form.card_style', 'form.font_family', 'form.avatar_shape', 'form.cover_position', 'form.density']],
                    'links' => ['label' => 'Redes Sociais', 'icon' => 'link-2', 'errors' => ['form.links']],
                    'privacidade' => ['label' => 'Privacidade & SEO', 'icon' => 'shield-check', 'errors' => ['form.visibility', 'form.seo_title', 'form.seo_description']],
                ];
            @endphp
        ...
        <div class="min-w-0 space-y-10">
            {{-- ==================== ABA: IDENTIDADE ==================== --}}
            <div x-show="tab === 'identidade'" x-cloak class="space-y-10">
                {{-- Visual Section --}}
            ...
            {{-- ==================== ABA: REDES SOCIAIS ==================== --}}
            <div x-show="tab === 'links'" x-cloak class="space-y-10">
            ...
            {{-- ==================== ABA: PRIVACIDADE & SEO ==================== --}}
            <div x-show="tab === 'privacidade'" x-cloak class="space-y-10">
