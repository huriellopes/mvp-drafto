...
<div {{ $attributes->merge(['class' => "relative inline-flex shrink-0 items-center justify-center overflow-hidden rounded-2xl bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 $sizeClass"]) }}>
    @if($src)
        <img src="{{ $src }}" alt="{{ $name }}" width="{{ $px }}" height="{{ $px }}" class="h-full w-full object-cover" loading="lazy" decoding="async">
    @else
        <span class="font-black uppercase tracking-widest text-zinc-600 dark:text-zinc-400">
            {{ get_initials($name) }}
        </span>
    @endif
</div>
